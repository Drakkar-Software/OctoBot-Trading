...
class OrdersParser(Parser):
    ...
    def _parse_timestamp(self, missing_timestamp_value):
        def found_timestamp(raw_timestamp):
    ...
    def _parse_type(self, missing_type_value=None):
        def missing_type(raw_order_type=None):
            if missing_type_value:
                ...
                if (
                    self.formatted_record[OrderCols.SIDE.value]
                    == TradeOrderSide.BUY.value
                ):
            ...
            if new_type:
                self.formatted_record[OrderCols.TYPE.value] = parse_exchange_order_type(
                    new_type
                )
                return new_type
            type_exists_message = (
            ...
            self._log_missing(
                ...
                f"{OrderCols.OCTOBOT_ORDER_TYPE.value} and based on taker_or_maker ({taker_or_maker or 'no taker_or_maker'})"
                f" which also failed to parse{type_exists_message}",
            )

        def parse_exchange_order_type(raw_order_type):
            ...
            self._log_missing(
                ...
                f"key: {OrderCols.OCTOBOT_ORDER_TYPE.value} got: {raw_order_type or 'no exchange order type'} which is not a TradeOrderType or TraderOrderType",
            )

        def type_found(raw_order_type):
            exchange_order_type = parse_exchange_order_type(raw_order_type)
            try:
            ...
            return missing_type(raw_order_type)

        self._try_to_find_and_parse_with_method(
            OrderCols.OCTOBOT_ORDER_TYPE.value,
            [OrderCols.TYPE.value],
            parse_method=type_found,
            not_found_method=missing_type,
            enable_log=False,
        )
    ...
    def _parse_filled_price(
        self,
    ):
        def missing_filled_price():
            # todo check if safe
        ...
        self._try_to_find_and_set_decimal(
            OrderCols.FILLED_PRICE.value,
            [OrderCols.FILLED_PRICE.value],
            not_found_method=missing_filled_price,
            enable_log=False,
        )
    ...
    def _parse_remaining(self):
        ...
        self._try_to_find_and_set_decimal(
            ...
            [OrderCols.REMAINING.value],
            not_found_method=missing_remaining,
        )
    ...
    def _parse_filled_amount(self):
        ...
        self._try_to_find_and_set_decimal(
            OrderCols.FILLED.value,
            [OrderCols.FILLED.value],
            not_found_method=missing_filled,
        )

    def _parse_fee(self):
        # todo test
        def found_fees(fees):
            if fees and OrderCols.COST.value in fees:
                fees[OrderCols.COST.value] = decimal.Decimal(
                    str(fees[OrderCols.COST.value])
                )
            return fees

        self._try_to_find_and_parse_with_method(
            OrderCols.FEE.value, [OrderCols.FEE.value], parse_method=found_fees
        )

    def _parse_taker_or_maker(self):
        def missing_taker_or_maker():
        ...
        self._try_to_find_and_set(
            OrderCols.TAKERORMAKER.value,
            [OrderCols.TAKERORMAKER.value],
            not_found_method=missing_taker_or_maker,
        )

    def _parse_reduce_only(self):
        def missing_reduce_only():
            if (
                self.formatted_record.get(OrderCols.OCTOBOT_ORDER_TYPE.value)
                == TradeOrderType.STOP_LOSS.value
            ):
                return True
            # dont raise as it's optional
        BYBIT_REDUCE_ONLY = "reduce_only"
        self._try_to_find_and_set(
            OrderCols.REDUCE_ONLY.value,
            [OrderCols.REDUCE_ONLY.value, BYBIT_REDUCE_ONLY],
            not_found_method=missing_reduce_only,
    ...
    def _parse_fees(self):
        ...
        self._try_to_find_and_parse_with_method(
            OrderCols.FEE.value,
            [OrderCols.FEE.value],
            parse_method=found_fees,
            not_found_method=missing_fees,
        )

    async def _fetch_if_missing(self):
        if (
            self.debugging_report_dict
            and (to_find_id := self.formatted_record.get(OrderCols.ID.value))
