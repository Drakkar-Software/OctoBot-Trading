...
def is_ms_valid(value, zero_valid=False):
    ...
    return (
        value is not None
        and value is not math.nan
        and (value >= 0 if zero_valid else value > 0)
    )


def check_market_status_limits(market_limit):
    return all(
        [check_market_status_values(market_limit[key].values()) for key in market_limit]
    )
...
def calculate_amounts(market_limit):
    ...
    if (
        not is_ms_valid(limit_amount[Ecmsc.LIMITS_AMOUNT_MAX.value])
        and Ecmsc.LIMITS_COST_MAX.value in limit_cost
        and Ecmsc.LIMITS_PRICE_MAX.value in limit_price
    ):
        if (
            is_ms_valid(limit_cost[Ecmsc.LIMITS_COST_MAX.value])
    ...
    if (
        not is_ms_valid(limit_amount[Ecmsc.LIMITS_AMOUNT_MIN.value])
        and Ecmsc.LIMITS_COST_MIN.value in limit_cost
        and Ecmsc.LIMITS_PRICE_MIN.value in limit_price
    ):
        if (
            is_ms_valid(limit_cost[Ecmsc.LIMITS_COST_MIN.value])
...
def calculate_costs(market_limit):
    ...
    if (
        not is_ms_valid(limit_cost[Ecmsc.LIMITS_COST_MAX.value])
        and Ecmsc.LIMITS_AMOUNT_MAX.value in limit_amount
        and Ecmsc.LIMITS_PRICE_MAX.value in limit_price
    ):
        if is_ms_valid(limit_amount[Ecmsc.LIMITS_AMOUNT_MAX.value]) and is_ms_valid(
            limit_price[Ecmsc.LIMITS_PRICE_MAX.value]
        ):
            limit_cost[Ecmsc.LIMITS_COST_MAX.value] = (
                limit_amount[Ecmsc.LIMITS_AMOUNT_MAX.value]
    ...
    if (
        not is_ms_valid(limit_cost[Ecmsc.LIMITS_COST_MIN.value])
        and Ecmsc.LIMITS_AMOUNT_MIN.value in limit_amount
        and Ecmsc.LIMITS_PRICE_MIN.value in limit_price
    ):
        if is_ms_valid(limit_amount[Ecmsc.LIMITS_AMOUNT_MIN.value]) and is_ms_valid(
            limit_price[Ecmsc.LIMITS_PRICE_MIN.value]
        ):
            limit_cost[Ecmsc.LIMITS_COST_MIN.value] = (
                limit_amount[Ecmsc.LIMITS_AMOUNT_MIN.value]
...
def calculate_prices(market_limit):
    ...
    if (
        not is_ms_valid(limit_price[Ecmsc.LIMITS_PRICE_MAX.value])
        and Ecmsc.LIMITS_COST_MAX.value in limit_cost
        and Ecmsc.LIMITS_AMOUNT_MAX.value in limit_amount
    ):
        if (
    ...
    if (
        not is_ms_valid(limit_price[Ecmsc.LIMITS_PRICE_MIN.value])
        and Ecmsc.LIMITS_COST_MIN.value in limit_cost
        and Ecmsc.LIMITS_AMOUNT_MIN.value in limit_amount
    ):
        if (
...
def fix_market_status_limits_from_current_data(market_limit):
    ...
    # calculate amounts
    if not (
        check_market_status_values(market_limit[Ecmsc.LIMITS_AMOUNT.value].values())
    ):
        calculate_amounts(market_limit)

    # calculate prices
    if not (
        check_market_status_values(market_limit[Ecmsc.LIMITS_PRICE.value].values())
    ):
...
class ExchangeMarketStatusParser:
        ...
        for key, val in element.items():
            if key in key_whitelist and isinstance(val, str):
                try:
                    ...
                    self.logger.debug(
                        f"Impossible to convert {val} to float in {key} market status. "
    ...
    def _fix_market_status_limits(self):
        ...
        if not check_market_status_limits(market_limit):
            ...
            if self.market_status_specific and not check_market_status_limits(
                market_limit
            ):
                self._fix_market_status_limits_with_specific()

            if self.price_example is not None and not check_market_status_limits(
                market_limit
            ):
    ...
    def _calculate_amount(self):
        ...
        if amount_log_price >= 0:
            amount_min = 10 ** (
                self.LIMIT_AMOUNT_MIN_SUP_ATTENUATION - amount_log_price
            )
            amount_max = 10 ** (
                self.LIMIT_AMOUNT_MAX_SUP_ATTENUATION - amount_log_price
            )
        else:
            amount_min = 10 ** -(amount_log_price + self.LIMIT_AMOUNT_MIN_ATTENUATION)
            amount_max = 10 ** (
                -amount_log_price + self.LIMIT_AMOUNT_MAX_MINUS_3_ATTENUATION
            )
            ...
            if Ecmsic.FILTERS.value in self.market_status_specific:
                ...
                for filter_dict in filters:
                    if Ecmsic.FILTER_TYPE.value in filter_dict:
                        if (
                            filter_dict[Ecmsic.FILTER_TYPE.value]
