...
class TradesParser(OrdersParser):
    """
    ...
    async def parse_trades(
        ...
        """
        check_completeness = check_completeness if check_completeness is not None else True
    ...
    async def parse_trade(
        ...
        await self.parse_order(
            raw_trade, status=OrderStatus.CLOSED.value, check_completeness=check_completeness
        )
