...
cdef class OrdersManager(util.Initializable):
    ...
    cpdef orders_personal_data.Order get_order(self, str order_id)
    cpdef bint upsert_order_instance(self, orders_personal_data.Order order)
    cpdef bint has_order(self, str order_id)
    cpdef void remove_order_instance(self, orders_personal_data.Order order)
