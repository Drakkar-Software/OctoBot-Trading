...
class ExchangePersonalData(util.Initializable):
    ...
    async def handle_portfolio_update(self, balance, should_notify: bool = True, is_diff_update=False) -> bool:
        try:
            ...
            if should_notify:
                await exchanges.get_chan(octobot_trading.constants.BALANCE_CHANNEL,
    ...
    async def handle_portfolio_update_from_order(self, order, should_notify: bool = True) -> bool:
        try:
            ...
            if should_notify:
                await exchanges.get_chan(octobot_trading.constants.BALANCE_CHANNEL, self.exchange_manager.id). \
    ...
    async def handle_portfolio_profitability_update(self, balance, mark_price, symbol, should_notify: bool = True):
        try:
            ...
            if should_notify:
                await exchanges.get_chan(octobot_trading.constants.BALANCE_PROFITABILITY_CHANNEL,
    ...
    async def handle_order_update_notification(self, order, is_new_order):
        ...
        """
        try:
            await exchanges.get_chan(octobot_trading.constants.ORDERS_CHANNEL,
                                     self.exchange_manager.id).get_internal_producer() \
    ...
    async def handle_trade_update(self, symbol, trade_id, trade, should_notify: bool = True):
        try:
            ...
            if should_notify:
                await exchanges.get_chan(octobot_trading.constants.TRADES_CHANNEL,
                                         self.exchange_manager.id).get_internal_producer() \
    ...
    async def handle_position_update(self, symbol, position_id, position, should_notify: bool = True):
        try:
            ...
            if should_notify:
                ...
                await exchanges.get_chan(octobot_trading.constants.POSITIONS_CHANNEL,
                                         self.exchange_manager.id).get_internal_producer() \
                    .send(cryptocurrency=self.exchange_manager.exchange.get_pair_cryptocurrency(symbol),
                          symbol=symbol,
