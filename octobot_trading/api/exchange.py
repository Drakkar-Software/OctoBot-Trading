...
def create_exchange_builder(config, exchange_name) -> object:
    return exchanges.ExchangeBuilder(config, exchange_name)
...
def get_exchange_configuration_from_exchange_id(exchange_id) -> object:
...
def get_exchange_configuration_from_exchange(exchange_name, exchange_id) -> object:
...
def has_only_ohlcv(exchange_importers):
    return exchanges.ExchangeSimulator.get_real_available_data(exchange_importers) == \
           set(producers.SIMULATOR_PRODUCERS_TO_POSSIBLE_DATA_TYPE[octobot_trading.OHLCV_CHANNEL])
