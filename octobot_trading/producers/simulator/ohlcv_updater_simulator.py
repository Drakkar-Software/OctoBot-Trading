...
class OHLCVUpdaterSimulator(OHLCVUpdater):
    def __init__(self, channel, importer):
        ...
        self.time_frames_to_second = {
            time_frame: TimeFramesMinutes[time_frame] * MINUTE_TO_SECONDS
        ...
        try:
            for time_frame in self.channel.exchange_manager.exchange_config.traded_time_frames:
                for pair in self.channel.exchange_manager.exchange_config.traded_symbol_pairs:
                    ...
                    # (selection is <= and >=)
                    # use timestamp + time_frame seconds to include the future candle that will be sorted in exchange
                    # simulator for later uses.
                    ohlcv_data: list = await self.exchange_data_importer.get_ohlcv_from_timestamps(
                        ...
                        superior_timestamp=timestamp + self.time_frames_to_second[time_frame])
                    if ohlcv_data:
                        ...
                        if ohlcv_data[0][-1][PriceIndexes.IND_PRICE_TIME.value] == timestamp:
                            ...
                            # if no future candle available (end of backtesting of missing data: reset future candle)
