...
from tests.orders import stop_loss_order
...
async def test_stop_loss_order_trigger(stop_loss_order):
    order_price = random_price()
    stop_loss_order.update(
        price=order_price,
        quantity=random_quantity(),
        symbol=DEFAULT_SYMBOL_ORDER,
        order_type=TradeOrderType.STOP_LOSS,
    )
    stop_loss_order.exchange_manager.is_backtesting = True  # force update_order_status
    await stop_loss_order.initialize()
    price_events_manager = stop_loss_order.exchange_manager.exchange_symbols_data.get_exchange_symbol_data(
        DEFAULT_SYMBOL_ORDER).price_events_manager
    price_events_manager.handle_recent_trades(
        [random_recent_trade(price=random_price(min_value=order_price + 1),
                             timestamp=stop_loss_order.timestamp)])
    await wait_asyncio_next_cycle()
    assert not stop_loss_order.is_filled()
    price_events_manager.handle_recent_trades(
        [random_recent_trade(price=order_price,
                             timestamp=stop_loss_order.timestamp - 1)])
    await wait_asyncio_next_cycle()
    assert not stop_loss_order.is_filled()
