...
async def test_is_authenticated(simulated_trader):
    _, exchange_manager, trader_inst = simulated_trader
    assert not exchange_manager.exchange.is_authenticated


async def test_get_uniform_timestamp(simulated_trader):
    _, exchange_manager, trader_inst = simulated_trader
    assert exchange_manager.exchange.get_uniform_timestamp(1e8) == 1e5


async def test_get_split_pair_from_exchange(simulated_trader):
    _, exchange_manager, trader_inst = simulated_trader
    assert exchange_manager.exchange.get_split_pair_from_exchange(DEFAULT_BACKTESTING_SYMBOL) == \
           DEFAULT_BACKTESTING_SPLIT_SYMBOL


async def test_get_pair_cryptocurrency(simulated_trader):
    _, exchange_manager, trader_inst = simulated_trader
...
async def test_get_trade_fee(simulated_trader):
    ...
    buy_limit_fee = exchange_manager.exchange.get_trade_fee(DEFAULT_BACKTESTING_SYMBOL, TraderOrderType.BUY_LIMIT, 10,
                                                            100, ExchangeConstantsMarketPropertyColumns.MAKER.value)
    _assert_fee(buy_limit_fee, DEFAULT_BACKTESTING_CURRENCY, 0.01, 0.001,
                ExchangeConstantsMarketPropertyColumns.MAKER.value)

    sell_limit_fee = exchange_manager.exchange.get_trade_fee(DEFAULT_BACKTESTING_SYMBOL, TraderOrderType.SELL_LIMIT, 10,
                                                             100, ExchangeConstantsMarketPropertyColumns.TAKER.value)
    _assert_fee(sell_limit_fee, DEFAULT_BACKTESTING_MARKET, 1, 0.001,
                ExchangeConstantsMarketPropertyColumns.TAKER.value)


async def test_stop(simulated_trader):
    _, exchange_manager, trader_inst = simulated_trader
