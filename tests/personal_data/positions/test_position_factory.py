...
async def test_create_position_instance_from_raw(future_trader_simulator):
    ...
    position = personal_data.create_position_instance_from_raw(trader_inst, DEFAULT_FUTURE_SYMBOL, raw_position)
    position_leverage = decimal_random_int(max_value=200)
    position_quantity = decimal_random_quantity(max_value=1000)
    linear_position_open = personal_data.create_position_instance_from_raw(trader_inst, DEFAULT_FUTURE_SYMBOL, {
        enums.ExchangeConstantsPositionColumns.SYMBOL.value: DEFAULT_FUTURE_SYMBOL,
    ...
    assert isinstance(linear_position_open, personal_data.LinearPosition)
