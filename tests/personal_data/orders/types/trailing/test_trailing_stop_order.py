...
from tests.test_utils.random_numbers import decimal_random_price, random_price, \
    decimal_random_quantity, random_recent_trade
...
async def test_trailing_stop_trigger(trailing_stop_order):
    ...
    print(f"min: {max_trailing_hit_price} max: {order_price - 1}")
    price_events_manager.handle_recent_trades(
        [random_recent_trade(price=random_price(min_value=float(max_trailing_hit_price), max_value=float(order_price - 1)),
    ...
    price_events_manager.handle_recent_trades([random_recent_trade(price=max_trailing_hit_price - decimal.Decimal("0.0001"),
                                                                   timestamp=trailing_stop_order.timestamp)])
    await wait_asyncio_next_cycle()
...
async def test_trailing_stop_with_new_price(trailing_stop_order):
    ...
    new_trailing_price = decimal_random_price(min_value=order_price * decimal.Decimal(str(0.99)), max_value=order_price *  decimal.Decimal(str(1.01)))
    ...
    # avoid decimal to float rounding issues
    price_events_manager.handle_recent_trades(
        [random_recent_trade(price=get_price_percent(new_trailing_price - decimal.Decimal("0.0001"),
...
async def test_trailing_stop_with_new_old_price(trailing_stop_order):
    ...
    price_events_manager.handle_recent_trades(
        [random_recent_trade(price=get_price_percent(order_price - decimal.Decimal("0.0001"),
                                                           trailing_stop_order.trailing_percent),
                             timestamp=trailing_stop_order.timestamp)])
    await wait_asyncio_next_cycle()
    assert trailing_stop_order.is_filled()
...
async def initialize_trailing_stop(order, side=TradeOrderSide.SELL) -> Tuple[TrailingStopOrder, float, PriceEventsManager]:
    ...
    order.update(
        ...
        quantity=decimal_random_quantity(max_value=order_max_quantity / 2),
...
def set_mark_price(order, mark_price):
    ...
    prices_manager.set_mark_price(float(mark_price), MarkPriceSources.EXCHANGE_MARK_PRICE.value)
