...
def test_update():
    ...
    assert not asset.update(available=constants.ZERO, total=constants.ZERO, margin_balance=constants.ZERO)
    assert asset.update(available=constants.ZERO, total=decimal.Decimal(5), margin_balance=decimal.Decimal(10))
    assert asset.total == decimal.Decimal(5)
    assert asset.margin_balance == decimal.Decimal(10)
    assert asset.update(available=constants.ZERO, total=constants.ZERO, margin_balance=decimal.Decimal(10),
                        maintenance_margin=decimal.Decimal(3))
    assert asset.margin_balance == decimal.Decimal(20)
    assert asset.total == decimal.Decimal(5)
...
def test_set():
    ...
    assert asset.total == decimal.Decimal(5)
    assert asset.margin_balance == decimal.Decimal(2)
...
def test_reset():
    asset = future_asset.FutureAsset(ASSET_CURRENCY_NAME,
                                     ...
                                     initial_margin=constants.ONE_HUNDRED, margin_balance=constants.ONE_HUNDRED,
                                     maintenance_margin=constants.ONE_HUNDRED,
                                     position_initial_margin=constants.ONE_HUNDRED)
    assert asset.available == asset.total == asset.initial_margin == asset.margin_balance == \
           asset.maintenance_margin == asset.position_initial_margin == constants.ONE_HUNDRED
    asset.reset()
    assert asset.available == asset.total == asset.initial_margin == asset.margin_balance == \
           asset.maintenance_margin == asset.position_initial_margin == constants.ZERO
